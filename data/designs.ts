export const dashboardDesigns = [
  {
    id: 1,
    title: "Analytics Dashboard",
    description: "Comprehensive analytics dashboard with real-time data visualization",
    emoji: "📊",
    category: "Dashboard",
    images: ["/images/worknova 1.png", "/images/worknova.png"],
    tools: ["Figma", "Adobe XD", "Sketch"],
    year: "2024",
    behanceUrl: "https://www.behance.net/gallery/analytics-dashboard"
  },
  {
    id: 2,
    title: "Sales Dashboard",
    description: "Sales performance tracking dashboard with KPI metrics",
    emoji: "📈",
    category: "Dashboard",
    images: ["/images/le bon.png", "/images/sales-2.jpg"],
    tools: ["Figma", "Principle"],
    year: "2024",
    behanceUrl: "https://www.behance.net/gallery/sales-dashboard"
  },
  {
    id: 3,
    title: "Admin Panel",
    description: "Clean and intuitive admin panel for content management",
    emoji: "⚙️",
    category: "Dashboard",
    images: ["/images/admin-1.jpg", "/images/admin-2.jpg"],
    tools: ["Figma", "Framer"],
    year: "2023",
    behanceUrl: "https://www.behance.net/gallery/admin-panel"
  }
];

export const mobileDesigns = [
  {
    id: 4,
    title: "E-commerce App",
    description: "Modern mobile shopping app with seamless user experience",
    emoji: "📱",
    category: "Mobile",
    images: ["/images/mobile-1.jpg", "/images/mobile-2.jpg"],
    tools: ["Figma", "Principle", "ProtoPie"],
    year: "2024",
    behanceUrl: "https://www.behance.net/gallery/ecommerce-app"
  },
  {
    id: 5,
    title: "Banking App",
    description: "Secure and user-friendly mobile banking application",
    emoji: "💳",
    category: "Mobile",
    images: ["/images/banking-1.jpg", "/images/banking-2.jpg"],
    tools: ["Figma", "Adobe XD"],
    year: "2024",
    behanceUrl: "https://www.behance.net/gallery/banking-app"
  },
  {
    id: 6,
    title: "Fitness Tracker",
    description: "Health and fitness tracking app with workout plans",
    emoji: "💪",
    category: "Mobile",
    images: ["/images/fitness-1.jpg", "/images/fitness-2.jpg"],
    tools: ["Figma", "Framer"],
    year: "2023",
    behanceUrl: "https://www.behance.net/gallery/fitness-tracker"
  }
];

export const webDesigns = [
  {
    id: 7,
    title: "SaaS Landing Page",
    description: "Converting landing page for B2B SaaS product",
    emoji: "💻",
    category: "Web",
    images: ["/images/saas-1.jpg", "/images/saas-2.jpg"],
    tools: ["Figma", "Webflow"],
    year: "2024",
    behanceUrl: "https://www.behance.net/gallery/saas-landing"
  },
  {
    id: 8,
    title: "Portfolio Website",
    description: "Creative portfolio website for digital agency",
    emoji: "🎨",
    category: "Web",
    images: ["/images/portfolio-1.jpg", "/images/portfolio-2.jpg"],
    tools: ["Figma", "Adobe XD"],
    year: "2024",
    behanceUrl: "https://www.behance.net/gallery/portfolio-website"
  },
  {
    id: 9,
    title: "E-learning Platform",
    description: "Educational platform with course management system",
    emoji: "📚",
    category: "Web",
    images: ["/images/elearning-1.jpg", "/images/elearning-2.jpg"],
    tools: ["Figma", "Sketch"],
    year: "2023",
    behanceUrl: "https://www.behance.net/gallery/elearning-platform"
  }
];